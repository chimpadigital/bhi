<template>
  <div>
    ...
    <div class="flex justify-between flex-col lg:flex-row  lg:items-center">
      <div class="title-bhi text-base lg:text-lg">
        Lista de pasajeros con reserva
      </div>

      <div class="btn-bhi-secondary-outline mt-4 lg:mt-0">
        Aprobar todos
      </div>
    </div>
...
export default {
  ...
  methods: {
    setPassanger (e) {
      localStorage.setItem('pasajero', JSON.stringify(e))
    },
    action (b) {
      this.$nuxt.$emit('OVERLAY_ACTIVE')
      this.$nuxt.$emit('ADMIN_ACTION_ACTIVE', b)
    }
  }
}
</script>
